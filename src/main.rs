...
fn main() -> Result<(), ArtopError> {
    ...
    let output_path = args.output_path.into_os_string().into_string()
    ...
    let mut converter: Converter<collect::ProcessorLoadReader, collect::MemoryUsageReader, collect::NetworkUsageReader> = Converter::<collect::ProcessorLoadReader, collect::MemoryUsageReader, collect::NetworkUsageReader>::new(&processor_load_reader, &memory_usage_reader, &network_usage_reader);

    let filler = match get_filler(&args.fill_method) {
        Some(filler) => filler,
        None         => return Err(ArtopError(format!("Given fill method \"{}\" does not exist", args.fill_method))),
    };
    ...
    let converter_error = match converter.run(DEFAULT_ROUNDS, DEFAULT_INTERVAL) {
        ...
        Ok(false) => ArtopError(String::from("The converter failed but did not report any error")),
...
fn get_filler(method: &String) -> Option<Filler> {
    match method.as_str() {
